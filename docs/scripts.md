...
当使用`player script`子命令加载该脚本后，会立即执行一遍`Init()`函数，然后根据命令传入的参数每隔`interval`gt执行一次`Tick()`函数，这称为脚本的一次**更新**。
...
执行指令 `/player bot scirpt info.lua 20` 即可让假人`bot`20gt执行一次`Tick()`函数，下同，不再赘述
